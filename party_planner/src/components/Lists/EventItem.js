...
const ListItem = (prop) => {
    const { itemType, item, updateItem, updateToDoList } = prop;
    ...
    const [radioStatus, setRadioStatus] = useState(
        item.purchased || item.completed ? checked : unchecked
    );
    ...
    return (
        <div className="item-container" aria-live="polite">
            <img
                src={radioStatus}
                alt="radio-button"
                onClick={() => handleUpdated(radioStatus)}
            />
            <p>{item.name}</p>
            {!edit && <p>{item.cost ? `$ ${item.cost}` : null}</p>}
            {!edit && isShopping ? (
                <Icon name="dollar" onClick={() => setEdit(true)} />
            ) : null}
            {isShopping && edit ? (
